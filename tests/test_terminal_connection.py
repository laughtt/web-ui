...
async def terminal_test(server_url):
    """
    Test the Local Terminal WebSocket endpoint.
                ...
                while True:
                    try:
                        ...
                        if msg_type == "connection_confirmed":
                            ...
                            logger.info(f"Starting local terminal session...")
                            connect_payload = {
                                "type": "connect",
                                "connection_name": "test_terminal"
                            }
                        ...
                        elif msg_type == "terminal_connected" or msg_type == "ssh_connected":
                            logger.info(f"✅ Terminal Connected: {data['data']['message']}")
                            nonlocal connection_name
                            connection_name = data['data']['connection_name']
                            logger.info("Type commands to execute. Type 'exit' to quit.")
                        ...
                        elif msg_type == "terminal_disconnected" or msg_type == "ssh_disconnected":
                            logger.info(f"Terminal connection {data['data']['connection_name']} disconnected")
                            
                        elif msg_type == "terminal_error" or msg_type == "ssh_error":
            ...
            try:
                ...
                while True:
                    ...
                    if not connection_name:
                        logger.debug("Waiting for terminal connection to be established...")
...
def main():
    parser = argparse.ArgumentParser(description="Test Terminal WebSocket")
    ...
    try:
        ...
        asyncio.run(terminal_test(args.server))
