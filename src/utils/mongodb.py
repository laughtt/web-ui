...
class MongoDB:
    ...
    def connect(self):
        ...
        try:
            mongo_uri = f"mongodb://{os.getenv('MONGODB_ROOT_USERNAME', 'root')}:{os.getenv('MONGODB_ROOT_PASSWORD', 'rootpassword')}@{os.getenv('MONGODB_HOST', 'localhost')}:{os.getenv('MONGODB_PORT', 27017)}/{os.getenv('MONGODB_DATABASE_NAME', 'brownser')}?authSource=admin"
            print(mongo_uri)
